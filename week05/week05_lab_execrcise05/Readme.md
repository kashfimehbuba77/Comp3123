...
-Ans: express.Router() is used to create modular, mountable route handlers. It helps organize your code by separating routes into different files.
...
-Ans: We can use try/catch blocks in routes and a central error-handling middleware to manage errors.
...
-Ans: process.env.PORT allows the app to use a port number provided by the hosting environment (e.g., Heroku, AWS). 8081 is the fallback local port if no environment variable is set.
